...
//	소켓을 보관하고 메세지를 주고 받기 위한 스트림을 보유
...
public class ServerMessegeThread extends Thread {
	...
	public ServerMessegeThread(MultiServer multiServer, Socket socket) {
//		서버를 통째로 가져와서 area를 만들어 로그를 기록해보자
...
//	듣는 메서드
	public void listen() {
		...
		try {
			msg = buffr.readLine();
			for(int i =0; i<multiServer.vec.size(); i++) { // 나를 포함한 반복문
				ServerMessegeThread smt = multiServer.vec.get(i);
			smt.sendMsg(msg); // 방금 수신한 메세지를 다시 클라이언트에게 보내기(출력)
//			다른 쓰레드의 sendMsg(msg)도 호출한다 // 다른 쓰레드는 Vector에 보관되어 있으므로 Vector에 접근해본다
			}
			
//			서버에 로그 출력
			multiServer.area.append(msg+"\n");
		} catch (IOException e) {
			flag = false; // 쓰레드 소멸시키기 위해
