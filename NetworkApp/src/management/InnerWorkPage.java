...
public class InnerWorkPage extends ProjectInnerPage implements ActionListener {
	DBManager dbManager = DBManager.getInstance();
	ProjectWorkDAO projectWorkDAO = new ProjectWorkDAO();
	ProjectWorkDTO projectWorkDTO = new ProjectWorkDTO();
	String currentTitle;
//	ProjectWorkDTO projectWorkDTO;
	...
	// 요일처리
	ProjectWorkDayCell[] dayCells = new ProjectWorkDayCell[7]; // 요일 모두 담기
	String[] dayTitle = { "일", "월", "화", "수", "목", "금", "토" };

	// 날짜
	ProjectWorkDateCell[][] dateCells = new ProjectWorkDateCell[6][7];

	// 현재 사용자가 보게될 날짜 정보!!
	Calendar currentObj = Calendar.getInstance();// 현재날짜
	// ----------------------------------------------------------------------------------------------

	public InnerWorkPage(ProjectMainPage projectMainPage) {
...
//		p_west.add(box_icon);
		...
		bt_prev.setPreferredSize(new Dimension(70, 40));
		...
		bt_next.setPreferredSize(new Dimension(70, 40));
		...
		p_title.setPreferredSize(new Dimension(1400, 50));
		...
		p_dayOfWeek.setBackground(Color.red);
		p_dayOfMonth.setPreferredSize(new Dimension(1400, 570));
		p_dayOfMonth.setBackground(Color.red);
...
//					int result = projectMainPage.projectWorkDAO.delete(projectWorkDTO.getProjectworkdiary_idx());
...
//
////						projectMainPage.mainPageShowHide(ProjectMainPage.INNERWORKPAGE);// 화면전환
//					}
//				}
//			}
//		});
...
//			dayCells[i]=dayCell;
			dayCells[i] = new ProjectWorkDayCell(dayTitle[i], "", 25, 20, 20); // 생성
	...
	public void createDayOfMonth() {
		...
		for (int i = 0; i < dateCells.length; i++) {// 층
			for (int a = 0; a < dateCells[i].length; a++) {// 층
				dateCells[i][a] = new ProjectWorkDateCell(this, "", "", 20, 40, 10);
	...
	public void printDate() {
		...
		for (int i = 0; i < dateCells.length; i++) {
			for (int a = 0; a < dateCells[i].length; a++) {
				ProjectWorkDateCell cell = dateCells[i][a];
	...
	public void printLog() {
		int yy = currentObj.get(Calendar.YEAR);
		int mm = currentObj.get(Calendar.MONTH);
		List<ProjectWorkDTO> diaryList = projectWorkDAO.selectAll(yy, mm);
...
//					불러온 데이터 만큼 반복문
					for (int x = 0; x < diaryList.size(); x++) {
						ProjectWorkDTO obj = diaryList.get(x); // 다이어리 한 건 추출
						if (date == obj.getDd()) {
//							해당 셀에 데이터 표현
//							dateCells[i][a].color = Color.GREEN;
	...
	public void regist() {
		...
		ProjectWorkDTO d = new ProjectWorkDTO();
...
//		d.setIcon(icon);
		...
		// insert();
		int result = projectWorkDAO.insert(d);
		if (result > 0) {// 성공이라면
...
//		// UnBoxing(객체형이 기본자료형으로 변경되는 것)
//		String content = (String) area.removeAll();
////		String icon = (String) box_icon.getSelectedItem();
//		
//		// 레코드 한껀 채워넣기
//		projectWorkDTO.setContent(content);
////		d.setIcon(icon);
//		
//		// insert();
//		int result = projectWorkDAO.delete(content);
//		if (result > 0) {// 성공이라면
...
//		}
	...
	// 셀을 선택할때 콤보박스의 값 변경
	public void setDateInfo(String title, ProjectWorkDTO projectWorkDTO) {
		this.projectWorkDTO = projectWorkDTO;
		...
		if (projectWorkDTO != null) {
			System.out.println("현재 pk : " + projectWorkDTO.getProjectworkdiary_idx());
			area.setText(projectWorkDTO.getContent());
			p_west.updateUI();
		}
	}
}