...
public class ProjectSalesDateCell extends ProjectSalesCell{
	Color color=Color.LIGHT_GRAY;
	InnerSalesPage innerSalesPage;
	
	public ProjectSalesDateCell(InnerSalesPage innerSalesPage, String title, String content, int fontSize, int x, int y) {
		super(title, content, fontSize, x, y);
		this.innerSalesPage = innerSalesPage;
...
//						콤보박스에 날짜 채워넣기 (콤보박스는 디자인이니까 디자인쪽에서 처리)
					ProjectSalesDTO dates = new ProjectSalesDTO();
					dates.setYy(innerSalesPage.currentObj.get(Calendar.YEAR));
					dates.setMm(innerSalesPage.currentObj.get(Calendar.MONTH));
					dates.setDd(Integer.parseInt(ProjectSalesDateCell.this.title));
					ProjectSalesDTO result = innerSalesPage.projectSalesDAO.selectByDates(dates);
					
					innerSalesPage.setDateInfo(ProjectSalesDateCell.this.title, result);
				} else {
